...
// 2. Add Typesense registered to the hybrid search registry (named export)
backend.add(
  import('@backstage-community/plugin-search-backend-module-typesense').then(
    m => ({ default: m.searchModuleTypesenseHybridSearch }),
  ),
);

// 3. Add other sub-engines (like Vertex AI Search)
backend.add(
