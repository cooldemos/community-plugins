# Vertex AI Search Backend Module for Backstage (`@nordnet/backstage-plugin-search-backend-module-vertexai`)

This backend module integrates Google Cloud Vertex AI Search (formerly Enterprise Search / Discovery Engine) capabilities into the Backstage Search system. It registers itself to the `@nordnet/backstage-plugin-search-backend-module-hybrid` router to handle specific categories (like `techdocs` and others).
...
> [!WARNING]
> **Important Ingestion Dependency**: Vertex AI Search is a query-only backend. It does not automatically parse TechDocs files. You **MUST** deploy and configure the companion ingestion module [`@nordnet/backstage-plugin-events-backend-module-gcs-eventarc`](../events-backend-module-gcs-eventarc/README.md) (or set up GCS bucket notifications) to upload and index documents when they are published.
...
| **Ingress Webhook** | `@nordnet/backstage-plugin-events-backend-module-gcs-eventarc` | Listens to Eventarc file creation triggers to sync updated HTML pages from GCS to the Vertex AI Search data store. |
