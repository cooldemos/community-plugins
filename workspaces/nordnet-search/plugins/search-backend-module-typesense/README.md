# Typesense Search Backend Module for Backstage (`@nordnet/backstage-plugin-search-backend-module-typesense`)

This backend module integrates Typesense search capabilities into the Backstage Search system. It registers itself to the `@nordnet/backstage-plugin-search-backend-module-hybrid` router to handle specific categories (like `software-catalog` and others).
